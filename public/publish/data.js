// Static data: races, floor themes, presets, status effects, traps, constants.
// Faithful port of the original simulator's lib/types.ts (adult fiction).
...
(function (D) {
  ...
  function getFloorTheme(floorThemes, floor) {
    ...
    for (var i = 0; i < FLOOR_THEMES.length; i++) {
      if (FLOOR_THEMES[i].name === name) return FLOOR_THEMES[i];
    }
    return FLOOR_THEMES[(Math.max(1, floor) - 1) % FLOOR_THEMES.length];
  }
  ...
  var PRESET_TRAPS = [
    { id: 'tentacle_pit', name: '触手深渊', category: '触手系', hint: '异形触手从地面、墙壁与天花板同时涌出，缠绕四肢并强制贯穿各个孔穴、注入黏滑体液' },
    { id: 'slime_swallow', name: '史莱姆吞噬', category: '粘液系', hint: '半透明史莱姆将全身包裹溶解衣物，粘液渗入每一寸肌肤与孔穴，持续震动摩擦敏感部位' },
    { id: 'pollen_garden', name: '催情花园', category: '催情植物', hint: '大片催情花朵喷出浓密花粉与花蜜，藤蔓束缚身体，花粉令理智迅速瓦解、身体极度敏感' },
    { id: 'lewd_rune', name: '淫纹拘束', category: '魔法拘束', hint: '地面浮现魔法淫纹，无形力量将身体固定成屈辱姿势，淫纹持续发烫、强制提升欲望与快感' },
    { id: 'succubus_mirror', name: '魅魔之镜', category: '催眠幻觉', hint: '古镜中浮现魅魔，以催眠话语支配意识，制造无法分辨真假的极淫幻觉并诱导主动堕落' },
    { id: 'parasite_egg', name: '寄生产卵', category: '寄生/产卵', hint: '寄生型生物钻入体内，向子宫与肠道注入大量卵，腹部隆起、被迫感受异物蠕动与产卵快感' },
    { id: 'ancient_machine', name: '远古拘束装置', category: '机械', hint: '遗迹中的机械装置自动启动，金属拘束具固定四肢，机械触手与按摩棒按程序持续强制高潮' },
    { id: 'orc_nest', name: '半兽人巢穴', category: '怪物巢穴', hint: '误入半兽人巢穴，被成群粗壮的半兽人按住轮流侵犯，浓厚精液灌满每一个孔穴' },
  ];

  var CHARACTER_PRESETS = [
    {
      name: '娜露梅亚', race: 'tauren', avatarUrl: './assets/narmaya.png',
    ...
    {
      name: '一之濑志希', race: 'human', avatarUrl: './assets/ichinose-shiki.png',
    ...
    {
      name: '桑山千雪', race: 'human', avatarUrl: './assets/kuwayama-chiyuki.png',
    ...
    {
      name: '鹭泽文香', race: 'human', avatarUrl: './assets/sagisawa-fumika.png',
  ...
  D.data = {
    TARGET_FLOOR: TARGET_FLOOR, RACE_INFO: RACE_INFO, FLOOR_THEMES: FLOOR_THEMES,
    BOSS_FLOOR_THEME: BOSS_FLOOR_THEME, rollFloorThemes: rollFloorThemes, getFloorTheme: getFloorTheme,
    PRESET_STATUS_EFFECTS: PRESET_STATUS_EFFECTS, PRESET_TRAPS: PRESET_TRAPS, CHARACTER_PRESETS: CHARACTER_PRESETS,
  };
})(window.Dungeon);
